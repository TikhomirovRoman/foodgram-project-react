...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        breakfast_tag = Tag.objects.create(name='завтрак',
                                           color='#9911AA',
                                           slug='breakfast')
        dinner_tag = Tag.objects.create(name='обед',
                                        color='#ffaa11',
                                        slug='dinner')
        drink_tag = Tag.objects.create(name='напиток',
                                       color='#11ffaa',
                                       slug='drink')
        desert_tag = Tag.objects.create(name='десерт',
                                        color='#bb3355',
                                        slug='desert')
        ...
        kompot = Recipe.objects.create(
            ...
            text='промойте ягоды, положите в кастрюлю с холодной водой \
                и доведите до кипения. Сразу после закипания снимите \
                кастрюлю с огня',
            cooking_time=10
        )
        water = Ingredient.objects.get(name='вода')
        redcurrant = Ingredient.objects.get(name='красная смородина')
        IngredientInRecipe.objects.create(recipe=kompot,
                                          ingredient=water,
                                          amount=500)
        IngredientInRecipe.objects.create(recipe=kompot,
                                          ingredient=redcurrant,
                                          amount=100)
        ...
        shashlik = Recipe.objects.create(
            ...
            text='купите мясо, замаринуйте его как-нибудь по вкусу и жарьте. \
                Пригласите друзей и вообще.',
            cooking_time=90
        )
        ...
        IngredientInRecipe.objects.create(recipe=shashlik,
                                          ingredient=meat,
                                          amount=1500)
        IngredientInRecipe.objects.create(recipe=shashlik,
                                          ingredient=onion,
        ...
        lemontea = Recipe.objects.create(
            ...
            text='Положите дольку лимона в чай. Через минуту добавьте сахар. \
                Не забудьте перемешать.',
            cooking_time=5
        )
        ...
        IngredientInRecipe.objects.create(recipe=lemontea,
                                          ingredient=tea,
                                          amount=1)
        IngredientInRecipe.objects.create(recipe=lemontea,
                                          ingredient=lemon,
        ...
        dumplings_ordinar = Recipe.objects.create(
            ...
            text='Купите в магазине пельмени, положете  кипящую воду. \
                Вскоре утопленники всплывут вверх брюхом. Спустя 10 минут \
                их можно вылавливать и есть. Не обожгитесь.',
            cooking_time=25
        )
        dumplings = Ingredient.objects.get(name='пельмени')
        IngredientInRecipe.objects.create(recipe=dumplings_ordinar,
        ...
        dumplings_fried = Recipe.objects.create(
            ...
            text='Возьмите из холодильника вареные пельмени, которые вы не \
                доели вчера. Положите на разогретую сковороду, смазанную \
                маслом. Жарьте до образования хрустящей корочки, положите \
                в тарелку и хрустите. бон апети',
            cooking_time=25
        )
        IngredientInRecipe.objects.create(recipe=dumplings_fried,
                                          ingredient=dumplings,
                                          amount=800)
        ...
        fried_eggs = Recipe.objects.create(
            ...
            text='Два яйца аккуратно разбейте и вылейте содержимое на \
                разогретую сковороду. Масло не забудьтеперед этим налить, а \
                то все прилипнет. Жарьте не накрывая крышкой. ',
            cooking_time=12
        )
        eggs = Ingredient.objects.get(name='яйца куриные')
        IngredientInRecipe.objects.create(recipe=fried_eggs,
                                          ingredient=eggs,
                                          amount=150)
        ...
        boiled_eggs = Recipe.objects.create(
            ...
            text='Положите яйцов холодную воду и поставте на огонь. \
                После закипания подождете 5 минут и снимайте с огня. \
                Обязательно почистите яцо перед употреблением в пищу. \
                Чтобы легче чистилось лучше сразу по готовности залить \
                яйцо холодной водой и остудить там.',
            cooking_time=10
        )
        IngredientInRecipe.objects.create(recipe=boiled_eggs,
                                          ingredient=eggs,
                                          amount=75)
        ...
        poached_egg = Recipe.objects.create(
            ...
            text='Разбейте яйцо в половник не повредив желток. Затем аккуратно\
                погрузите половник вместе с яйцом в кипящую воду и варите 1 \
                минуту. Выловите яйцо и срочно на блюдо!',
            cooking_time=10
        )
        IngredientInRecipe.objects.create(recipe=poached_egg,
                                          ingredient=eggs,
                                          amount=75)
        ...
        mulled_wine = Recipe.objects.create(
            ...
            text='Закиньте ингридиенты в вино и нагрейте на огне до 80 \
                градусов. Желательно не закипятить. После нагрева дайте \
                напитку настояться в закрытой кастрюле, а лучше в термосе.',
            cooking_time=15
        )
        vine = Ingredient.objects.get(name='вино красное сладкое')
        cinnamon = Ingredient.objects.get(name='корица')
        IngredientInRecipe.objects.create(recipe=mulled_wine,
                                          ingredient=vine,
                                          amount=700)
        IngredientInRecipe.objects.create(recipe=mulled_wine,
                                          ingredient=cinnamon,
        ...
        boiled_potato = Recipe.objects.create(
            ...
            text='Тщательно помойте картофель, положите в холодную воду и \
                варите на среднем огне пока нож не будет с легкостью \
                протыкать картофель на 1 см в глубину. Как потыкаете, \
                сливайте воду и тушите свет.',
            cooking_time=30
        )
        potatoes = Ingredient.objects.get(name='картофель')
        IngredientInRecipe.objects.create(recipe=boiled_potato,
                                          ingredient=potatoes,
                                          amount=1000)
        ...
        IngredientInRecipe.objects.create(recipe=milkcorn,
                                          ingredient=flakes,
                                          amount=200)
        IngredientInRecipe.objects.create(recipe=milkcorn,
                                          ingredient=milk,
